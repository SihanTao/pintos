...
static void check_ranged_memory (const void *start, size_t length,
                                 size_t size_of_type);
...
static syscall_func_t syscall_funcs[] = {
  ...
  [SYS_READ] = sys_read_handler,     [SYS_WRITE] = sys_write_handler,
  [SYS_SEEK] = sys_seek_handler,     [SYS_TELL] = sys_tell_handler,
  [SYS_CLOSE] = sys_close_handler
};

static int argc_syscall[]
    = { [SYS_HALT] = 0,   [SYS_EXIT] = 1,   [SYS_EXEC] = 1, [SYS_WAIT] = 1,
        [SYS_CREATE] = 2, [SYS_REMOVE] = 1, [SYS_OPEN] = 1, [SYS_FILESIZE] = 1,
        [SYS_READ] = 3,   [SYS_WRITE] = 3,  [SYS_SEEK] = 2, [SYS_TELL] = 1,
        [SYS_CLOSE] = 1 };
...
syscall_handler (struct intr_frame *f UNUSED)
{
  ...
  check_and_resolve_syscall_stack (argc_syscall[syscall_number], stack_ptr,
                                   sys_argv);
  f->eax
      = syscall_funcs[syscall_number](sys_argv[0], sys_argv[1], sys_argv[2]);
}
 ...
 */
void
check_safe_memory_access (const void *vaddr)
{
...
static int
sys_write_handler (int fd, int buffer, int size)
{
...
static int
sys_halt_handler (int arg0 UNUSED, int arg1 UNUSED, int arg2 UNUSED)
{
  shutdown_power_off ();
  NOT_REACHED ();
}
...
int
sys_exit_handler (int exit_status, int arg1 UNUSED, int arg2 UNUSED)
{
...
static int
sys_exec_handler (int cmd_line, int arg1 UNUSED, int arg2 UNUSED)
{
...
static int
sys_wait_handler (int pid, int arg1 UNUSED, int arg2 UNUSED)
{
  return process_wait ((tid_t)pid);
}

/* Check if the file has a valid virtual address and call filesys_create to
 * create the file.*/
static int
sys_create_handler (int file_name, int size, int arg2 UNUSED)
{
...
/* Check if the file has a valid virtual address and call filesys_remove to
 * remove */
static int
sys_remove_handler (int file_name, int arg1 UNUSED, int arg2 UNUSED)
{
  check_string_memory ((const char *)file_name);
...
int
sys_open_handler (int file_name, int arg1 UNUSED, int arg2 UNUSED)
{
  check_string_memory ((const char *)file_name);
...
static int
sys_filesize_handler (int fd, int arg1 UNUSED, int arg2 UNUSED)
{
  // only one thread can access to its file descriptor list, thus
  // doesn't need to be protected by locks
  struct file *file = to_file (fd);
...
*/
static int
sys_read_handler (int fd, int buffer, int size)
{
  check_ranged_memory ((char *)buffer, size, sizeof (char));
...
static int
sys_seek_handler (int fd, int position, int arg2 UNUSED)
{
...
static int
sys_tell_handler (int fd, int arg1 UNUSED, int arg2 UNUSED)
{
  if (fd == STDIN_FILENO || fd == STDOUT_FILENO)
    return 0;
...
static int
sys_close_handler (int fd, int arg1 UNUSED, int arg2 UNUSED)
{
...
inline struct file_descriptor *
to_file_descriptor (int fd)
{
  struct thread *cur = thread_current ();
...
void
check_string_memory (const char *start)
{
...
// thus doesn't need to be protected by locks
void
check_ranged_memory (const void *start, size_t length, size_t size_of_type)
{
  check_safe_memory_access (start);
